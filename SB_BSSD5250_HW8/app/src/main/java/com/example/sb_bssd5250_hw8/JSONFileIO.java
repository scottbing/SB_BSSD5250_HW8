...
public class JSONFileIO {
    ...
    public JSONFileIO() {
        ...
        makeData();
        //writeDataFile();
        NoteJSONSerializer noteJSONSerializer = new NoteJSONSerializer( mAppContext, "notes.json" );
        try {
            noteJSONSerializer.saveNotes (NotesData.getInstance(mAppContext).getNoteList());
        }  catch (Exception e) {
    ...
    private void makeData() {
        ArrayList<Note> notes = NotesData.getInstance(mAppContext).getNoteList();
        for(int i=0;  i<10;  i++) {
            Note note = new Note();
            note.setName("Note	#" + i);
